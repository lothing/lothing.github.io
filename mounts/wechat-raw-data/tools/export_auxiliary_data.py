...
def export_contacts(db_path, out_dir):
    with connect(db_path) as conn:
        ...
        if table_exists(conn, "contact_label"):
            count_labels = write_jsonl(
                ...
                (row_to_jsonable(r) for r in conn.execute("SELECT * FROM contact_label")),
            )
        else:
            count_labels = 0
        if table_exists(conn, "chatroom_member"):
            count_members = write_jsonl(
                ...
                (row_to_jsonable(r) for r in conn.execute("SELECT * FROM chatroom_member")),
            )
        else:
            count_members = 0
...
def export_favorites(db_path, out_dir):
    with connect(db_path) as conn:
...
def export_sns(db_path, out_dir):
    counts = {}
    with connect(db_path) as conn:
...
def export_message_resource(db_path, out_dir):
    with connect(db_path) as conn:
        table_names = [
            r["name"]
        ...
        for table in table_names:
            ...
            def rows(table=table):
                for r in conn.execute(f"SELECT * FROM [{table}]"):
                    yield row_to_jsonable(r)
            counts[table] = write_jsonl(out_dir / f"message_resource__{table}.jsonl", rows())
    return counts


def generic_dump(db_path, out_dir, rel_key, max_rows):
    with connect(db_path) as conn:
        schema = [
            dict(r)
    ...
    for rel in targets:
        ...
        try:
            if rel in specialized:
                result = specialized[rel](db_path, out_dir)
            else:
                result = generic_dump(db_path, out_dir, rel, args.generic_max_rows)
